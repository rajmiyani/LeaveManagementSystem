...
function getManagerToken() {
    ...
    try {
        ...
        if (Array.isArray(parsed)) {
            const managerEntry = parsed.find(t => t.role === "manager");
            return managerEntry?.token || "";
        }
        return parsed.token || "";
    } catch {
        return "";
...
export default function ProfileSettings() {
    ...
    useEffect(() => {
        ...
        if (!token) {
            console.error("No token found for manager in localStorage");
            return;
        }
        console.log(token);


        axios.get("http://localhost:8080/manager/profile", {
            ...
            .then((res) => {
                ...
                setProfileImage(data.profile_image ? data.profile_image : Profile);
            })
            .catch((err) => {
    ...
    const handleSave = () => {
        ...
        axios.post("http://localhost:8080/manager/profile/update", form, {
            headers: {
            ...
            .then((res) => {
                ...
                if (updated) {
                    // update local state
                    setFormData(updated);
                    setProfileImage(updated.profile_image ? updated.profile_image : Profile);
                    
                    // store updated profile in localStorage
                    localStorage.setItem("ManagerProfileData", JSON.stringify(updated));
